...
export const Success = ({ widget }: CellSuccessProps<EditWidgetById>) => {
  const [updateWidget, { loading, error }] = useMutation(UPDATE_WIDGET_MUTATION, {
    onCompleted: () => {
  ...
  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">Edit Widget {widget.id}</h2>
      </header>
      <div className="rw-segment-main">
        <WidgetForm widget={widget} onSave={onSave} error={error} loading={loading} />
      </div>
    </div>
  )
}
