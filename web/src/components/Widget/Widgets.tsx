...
import { QUERY } from 'src/components/Widget/WidgetsCell'
    ...
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        ...
        <tbody>
          {widgets.map((widget) => (
            <tr key={widget.id}>
              <td>{truncate(widget.id)}</td>
              <td>{truncate(widget.name)}</td>
              <td>{truncate(widget.description)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.widget({ id: widget.id })}
                    title={'Show widget ' + widget.id + ' detail'}
